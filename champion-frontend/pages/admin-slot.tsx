...
const GET_SLOTS = gql`
  query GetSlots($fieldId: ID!, $tanggal: String!) {
...
export default function AdminSlot() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      setJamSelesai("");
      await refetchSlots();
      // pastikan semua query aktif direfresh sehingga halaman index (a & b) melihat perubahan
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f8fafc" }}>
      ...
      <div style={{ flex: 1, padding: "32px" }}>
        <h1 style={{ fontSize: 24, marginBottom: 20, color: "#0f172a" }}>
        ...
        {fieldId && tanggal && (
          <div>
            <h2 style={{ marginBottom: 10, color: "#1e3a8a" }}>
              Daftar Slot ({tanggal})
            </h2>
            {!slotData ? (
                ...
                <tbody>
                  {slotData.slots.map((slot: any, i: number) => (
                    <tr
                      ...
                      style={{
                        background: 
                          slot.status === "expired" ? "#fecaca" :
                          i % 2 === 0 ? "#f9fafb" : "#fff",
                        borderBottom: "1px solid #e2e8f0",
                        opacity: slot.status === "expired" ? 0.7 : 1,
                      }}
                    >
