...
function ReservationForm({ field, slot, onClose }: any) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    setLoading(true);
    try {
      // Generate email dari nama (untuk membuat unique email)
      const emailGenerated = `${nama.toLowerCase().replace(/\s+/g, ".")}.${Date.now()}@reservasi.local`;
      const passwordGenerated = Math.random().toString(36).substring(2, 10);
      ...
      setSuccess("Reservasi berhasil! Silakan simpan bukti Anda atau cetak.");
      setLastReservation(reservationData.createReservation);

      // buat tampilan bukti reservasi yang dapat dicetak atau disimpan sebagai PDF
      const res = reservationData && reservationData.createReservation;
      const html = `
        <html>
        <head>
          ...
          <style>
            ...
            .card{ border:1px solid #ddd; padding:20px; border-radius:8px; max-width:540px; margin:0 auto }
            h2{ margin-top:0 }
            .row{ display:flex; justify-content:space-between; margin:8px 0 }
            .muted{ color:#666 }
            .print-btn{ margin-top:18px; padding:10px 16px; background:#1976d2; color:#fff; border:none; border-radius:6px; cursor:pointer }
          </style>
        </head>
        <body>
          <div class="card">
            <h2>Bukti Reservasi</h2>
            <div class="row"><div class="muted">Nama</div><div>${nama}</div></div>
            <div class="row"><div class="muted">Kontak</div><div>${telepon}</div></div>
            <div class="row"><div class="muted">Lapangan</div><div>${field.nama_lapangan}</div></div>
            <div class="row"><div class="muted">Tanggal</div><div>${slot.tanggal}</div></div>
            <div class="row"><div class="muted">Jam</div><div>${slot.jam_mulai} - ${slot.jam_selesai}</div></div>
            <div class="row"><div class="muted">Status</div><div>${res ? res.status : "-"}</div></div>
            <div style="margin-top:12px; font-size:13px; color:#444">Tunjukkan bukti ini kepada petugas saat diminta.</div>
      ...
      if (w) {
        ...
        w.document.close();
        w.focus();
        // jangan paksa print langsung; beri sedikit delay supaya dialog muncul jika browser memperbolehkan
        setTimeout(() => { try { w.print(); } catch (e) {} }, 500);
      }
      alert("Reservasi berhasil! Bukti terbuka di jendela baru untuk dicetak atau disimpan sebagai PDF. Hubungi admin: 08123456789");
    } catch (err: any) {
      ...
      {success && (
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ marginTop: 12 }}>
            <button style={styles.primaryBtn} onClick={openReceipt}>Cetak Bukti (PDF)</button>
          </div>
        </div>
      )}
